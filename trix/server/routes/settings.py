...
def get_setting(key: str, default: Any = None) -> Any:
    ...
    with db.session() as session:
        from trix.storage.models import Setting
...
async def test_llm_connection():
    ...
    try:
        from trix.llm.config import LLMConfig as TrixLLMConfig
        from trix.llm.llm import LLM
        
        # Get current config
        model = get_setting("llm.model", os.environ.get("STRIX_LLM", "openai/gpt-4o"))
        timeout = int(get_setting("llm.timeout", 600) or 600)
        
        config = TrixLLMConfig(
            model_name=model,
        ...
        # Simple test - use message list format (generate expects list[dict])
        test_message = [{"role": "user", "content": "Say 'Hello from Trix!' in exactly those words."}]
        response = await llm.generate(test_message)
        
        return {
            "status": "success",
            "model": model,
            "response": response.content[:100] if response and response.content else None,
        }
